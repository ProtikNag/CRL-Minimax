...
| `experts/<Game>/` | **Precomputed single-task experts** (Impala-CNN). Reward curves + `experts_summary.csv` budget table (best greedy score + frames/episodes/iters-to-converge, for a matched CLEAR budget). Model binaries stay under `experts/` (gitignored); this is the published gallery. `python -m experiments.experts_summary`. | How good is each single-task expert, and how many frames did it take? (the fixed local references + fair-budget accounting) |

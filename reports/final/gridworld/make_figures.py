...
# ── Figure 6: what forward transfer is measuring ────────────────────────────
#
# Rows of this figure. Ours is the subject, CKA-RL is the headline rival, CReLUs
# holds the best forward transfer of anything we ran. CbpNet and fine-tuning are
# left out for space and sit between CKA-RL and CReLUs.
FWT_ROWS = ("ours", "cka_rl", "crelu")
FWT_SEED = "seed0"   # one seed, so the curves are real runs rather than a mean


def phase_curve(run: str, task: int) -> tuple[np.ndarray, np.ndarray]:
    """Task's own learning curve during its own phase, x normalised to [0, 1].

    x is the fraction of *that method's* phase for that task, because the metric
    averages each method over its own phase; the phases are not the same length.
    """
    points = []
    with (RUNS / run / "learning_curves.csv").open() as handle:
        for row in csv.DictReader(handle):
            if int(row["task"]) == task:
                points.append((int(row["iter"]), float(row["normalized"])))
    points.sort()
    iters = np.array([i for i, _v in points], dtype=float)
    scores = np.array([v for _i, v in points])
    return (iters - iters[0]) / max(iters[-1] - iters[0], 1e-9), scores


def auc_of(run: str, task: int) -> float:
    """The metric's AUC: the mean of the within-phase eval scores.

    Mirrors ``analysis/contract_metrics.within_phase_auc``. Evals are evenly
    spaced within a phase, so the mean is the area under the normalised curve.
    """
    return float(phase_curve(run, task)[1].mean())


def representative_tasks(count: int = 4) -> list[int]:
    """One task per equal slice of the sequence, chosen to be representative.

    Among all such picks, take the one whose per-method mean forward transfer
    reproduces each method's own overall value most closely. Four tasks picked
    for looking good would be four tasks picked to make a point; this rule is
    stated in the caption and is the reason the panels can be read as typical.
    """
    from itertools import product

    base = f"biggrid50_sh_baseline_{FWT_SEED}"
    runs = {key: f"{prefix}{FWT_SEED}" for key, _l, _c, prefix in METHODS
            if key in FWT_ROWS}
    per_task = {}
    for key, run in runs.items():
        per_task[key] = {t: forward_transfer(run, base, t) for t in range(N_TASKS)}
    overall = {key: np.mean(list(v.values())) for key, v in per_task.items()}

    edges = np.linspace(0, N_TASKS, count + 1).astype(int)
    slices = [range(edges[i], edges[i + 1]) for i in range(count)]
    best, chosen = np.inf, None
    for combo in product(*slices):
        error = max(abs(np.mean([per_task[k][t] for t in combo]) - overall[k])
                    for k in per_task)
        if error < best:
            best, chosen = error, combo
    return list(chosen)


def forward_transfer(run: str, base: str, task: int) -> float:
    """Per-task forward transfer, eq. 9: (AUC − AUC_b) / (1 − AUC_b)."""
    auc_b = auc_of(base, task)
    return (auc_of(run, task) - auc_b) / (1 - auc_b) if abs(1 - auc_b) > 1e-6 else 0.0


def figure_forward_transfer() -> None:
    """Forward transfer drawn as the two areas it is a ratio of.

    Every other figure here reports forward transfer as a number, and a number
    hides that the metric is a comparison of two learning curves. Eq. 9 is
    ``(AUC − AUC_b) / (1 − AUC_b)``, and both terms are areas on the same axes:
    the numerator is the band between the method and a from-scratch run on the
    same task, the denominator is everything between that from-scratch run and a
    solved task. Forward transfer is the share of the room the baseline left that
    the method actually takes.

    Drawn rather than asserted, this makes ours' last place legible. Ours does
    not fail to learn these tasks; it spends less of its budget inside each
    task's own phase and makes it up during consolidation, which this metric by
    construction cannot see.
    """
    tasks = representative_tasks()
    rows = [m for m in METHODS if m[0] in FWT_ROWS]
    base = f"biggrid50_sh_baseline_{FWT_SEED}"

    n_rows, n_cols = len(rows), len(tasks)
    left_m, right_m, top_m = 96, 24, 104
    gap_w, gap_v, panel_h = 26, 34, 112
    plot_w = W_FULL - left_m - right_m
    plot_h = n_rows * panel_h + (n_rows - 1) * gap_v
    ...
    for r, (key, label, color, prefix) in enumerate(rows, start=1):
        run = f"{prefix}{FWT_SEED}"
        for c, task in enumerate(tasks, start=1):
            mx, my = phase_curve(run, task)
            bx, by = phase_curve(base, task)
            method = np.interp(grid, mx, my)
            baseline = np.interp(grid, bx, by)

            def band(low: np.ndarray, high: np.ndarray, fill: str) -> None:
                fig.add_trace(go.Scatter(
            ...
            # Denominator first, so the numerator sits on top of it.
            band(baseline, np.ones_like(grid), hex_to_rgba(AC["axis"], 0.07))
            # Splitting on the sign costs nothing and a shortfall reads as a
            # shortfall: np.maximum leaves the losing stretches zero-height.
            band(baseline, np.maximum(method, baseline),
                 hex_to_rgba(color, 0.30))
            band(np.minimum(method, baseline), baseline,
                 hex_to_rgba(AC["red"], 0.22))

            fig.add_trace(go.Scatter(
                x=grid, y=np.ones_like(grid), mode="lines",
                line=dict(color=AC["border"], width=1, dash="dot"),
                hoverinfo="skip", showlegend=False), row=r, col=c)
            fig.add_trace(go.Scatter(
                x=bx, y=by, mode="lines",
                line=dict(color=AC["text_faint"], width=1.6, dash="dash"),
                hoverinfo="skip", showlegend=False), row=r, col=c)
            fig.add_trace(go.Scatter(
            ...
            index = (r - 1) * n_cols + (c - 1)
            axis = f"{index + 1 if index else ''}"
            value = forward_transfer(run, base, task)
            fig.add_annotation(
                x=0, y=1.0, xref=f"x{axis} domain", yref=f"y{axis} domain",
                yshift=7, text=f"<b>FT {value:+.2f}</b>", showarrow=False,
                xanchor="left", yanchor="bottom",
                font=dict(family=FONT_MONO, size=FS_NOTE,
                          color=color if value >= 0 else AC["red"]))
            if r == 1:
                fig.add_annotation(
                    x=0.5, y=1.0, xref=f"x{axis} domain", yref=f"y{axis} domain",
                    yshift=28, text=f"<b>Task {task}</b>", showarrow=False,
                    xanchor="center", yanchor="bottom",
                    font=dict(family=FONT_UI, size=FS_AXIS,
                              color=AC["text_primary"]))
            if c == 1:
                fig.add_annotation(
                    x=0, y=0.5, xref="paper", yref=f"y{axis} domain",
                    xshift=-(left_m - 10), text=f"<b>{label}</b>",
                    showarrow=False, textangle=-90, xanchor="center",
                    yanchor="middle",
                    font=dict(family=FONT_UI, size=FS_TICK, color=color))

    # Legend entries carry the meaning of the two bands; nothing is written on
    # the artwork itself.
    for name, fill in (
            ("<b>captured</b>, AUC − AUC<sub>base</sub>",
             hex_to_rgba(AC["blue"], 0.30)),
            ("<b>available</b>, 1 − AUC<sub>base</sub>",
             hex_to_rgba(AC["axis"], 0.07)),
            ("shortfall below the baseline", hex_to_rgba(AC["red"], 0.22))):
        fig.add_trace(go.Scatter(
            x=[None], y=[None], mode="markers", name=name,
            marker=dict(size=13, symbol="square", color=fill,
                        line=dict(color=hex_to_rgba(AC["axis"], 0.35), width=1)),
            showlegend=True, hoverinfo="skip"), row=1, col=1)
    fig.add_trace(go.Scatter(
        x=[None], y=[None], mode="lines", name="learned from scratch",
        line=dict(color=AC["text_faint"], width=1.6, dash="dash"),
        showlegend=True, hoverinfo="skip"), row=1, col=1)

    fig.update_xaxes(range=[0, 1], dtick=0.5, showgrid=False, zeroline=False,
                     showline=True, linecolor=AC["axis"], linewidth=1.2,
                     ticklen=4, showticklabels=False,
                     tickfont=dict(family=FONT_MONO, size=FS_NOTE,
                                   color=AC["text_muted"]))
    fig.update_yaxes(range=[-0.12, 1.10], dtick=0.5, showgrid=True,
                     gridcolor=AC["grid"], gridwidth=0.6, zeroline=False,
                     showline=True, linecolor=AC["axis"], linewidth=1.2,
                     ticklen=4, showticklabels=False,
                     tickfont=dict(family=FONT_MONO, size=FS_NOTE,
                                   color=AC["text_muted"]))
    fig.update_xaxes(showticklabels=True, row=n_rows)
    fig.update_yaxes(showticklabels=True, col=1)

    title = dict(font=dict(family=FONT_UI, size=FS_AXIS,
                           color=AC["text_primary"]))
    fig.add_annotation(
        x=0.5, y=0, xref="paper", yref="paper", yshift=-38,
        text="fraction of the task's own learning phase", showarrow=False,
        xanchor="center", yanchor="top", **title)
    fig.add_annotation(
        x=0, y=0.5, xref="paper", yref="paper", xshift=-(left_m - 44),
        text="normalised score", showarrow=False, textangle=-90,
        xanchor="center", yanchor="middle", **title)

    overall = ", ".join(
        f"{label.split(' (')[0]} {np.mean([forward_transfer(f'{prefix}{FWT_SEED}', base, t) for t in range(N_TASKS)]):+.2f}"
        for _k, label, _c, prefix in rows)
    bottom_m = add_footnote(fig, (
        "<b>Forward transfer is a ratio of the two shaded areas</b>, eq. 9 of "
        "the CKA-RL paper:<br>"
        "(AUC − AUC<sub>base</sub>) / (1 − AUC<sub>base</sub>), where AUC is "
        "the mean normalised score over a task's own learning phase.<br>"
        "So AUC is the average height of these curves, and forward transfer is "
        "the share of the room the baseline left that the method takes.<br>"
        "<b>x is normalised per method</b>, because the metric averages each "
        "method over its own phase and those phases differ in length.<br>"
        "That is what the metric does, and it is also what it hides.<br>"
        "Four tasks, one per quarter of the sequence, chosen so their mean "
        "forward transfer reproduces each method's own:<br>"
        f"{overall} over all 50 tasks.<br>"
        f"One seed ({FWT_SEED}), so these are real runs rather than an average "
        "of curves.<br>"
        "CbpNet (+0.18) and fine-tuning (+0.14) are omitted for space and sit "
        "between CKA-RL and CReLUs."), left_m)
    fig.update_layout(
        title=None, plot_bgcolor=AC["bg"],
        margin=dict(l=left_m, r=right_m, t=top_m, b=bottom_m),
        legend=dict(orientation="h", x=0.0, xanchor="left",
                    y=1 + 78 / plot_h, yanchor="bottom",
                    bgcolor="rgba(0,0,0,0)", borderwidth=0,
                    itemsizing="constant",
                    font=dict(family=FONT_UI, size=FS_NOTE,
                              color=AC["text_primary"])),
        showlegend=True)
    export_pair(fig, "forward_transfer", W_FULL,
                round(plot_h) + top_m + bottom_m)


# ── Figure 7: how much of the forward-transfer gap is real ──────────────────
def figure_forward_transfer_spread() -> None:
    """Every task's forward transfer, per method, as a strip.

    The companion to the figure above, and the reason not to lean on the
    forward-transfer column. The per-task spread is far wider than the gap
    between the methods' means, and the distributions overlap almost completely:
    ours and fine-tuning have the same median to two decimals despite a 0.03
    difference in mean. Showing the points is what keeps a 0.17 gap in a column
    of means from being read as a clean separation.
    """
    base = f"biggrid50_sh_baseline_{FWT_SEED}"
    rows = [m for m in METHODS if m[0] not in ("baseline",)]

    fig = go.Figure()
    floor = 0.0   # lowest per-task value drawn, so nothing falls off the axis
    for index, (key, label, color, prefix) in enumerate(rows):
        run = f"{prefix}{FWT_SEED}"
        values = np.array([forward_transfer(run, base, t) for t in range(N_TASKS)])
        floor = min(floor, float(values.min()))
        jitter = (np.random.default_rng(index).random(len(values)) - 0.5) * 0.30
        fig.add_trace(go.Scatter(
            x=values, y=index + jitter, mode="markers",
            marker=dict(color=hex_to_rgba(color, 0.35), size=5.5,
                        line=dict(width=0)),
            hovertemplate="%{x:+.2f}<extra></extra>", showlegend=False))
        # The metric is a mean over tasks, so the mean is what gets the rule.
        mean = float(values.mean())
        fig.add_trace(go.Scatter(
            x=[mean, mean], y=[index - 0.32, index + 0.32], mode="lines",
            line=dict(color=color, width=4.5),
            hoverinfo="skip", showlegend=False))
        fig.add_annotation(
            x=mean, y=index - 0.34, text=f"<b>{mean:+.2f}</b>", showarrow=False,
            xanchor="center", yanchor="bottom", yshift=3,
            font=dict(family=FONT_MONO, size=FS_VALUE, color=color),
            bgcolor="rgba(255,255,255,0.90)", borderpad=2)
        fig.add_annotation(
            x=1.04, y=index, text=f"{int((values < 0).sum())} of {N_TASKS}",
            showarrow=False, xanchor="left", yanchor="middle",
            font=dict(family=FONT_UI, size=FS_NOTE, color=AC["text_muted"]))

    fig.add_shape(type="line", x0=0, x1=0, y0=-0.75, y1=len(rows) - 0.4,
                  line=dict(color=AC["axis"], width=1.1, dash="dot"),
                  layer="below")
    fig.add_annotation(
        x=1.04, y=-0.72, text="<i>tasks made worse</i>", showarrow=False,
        xanchor="left", yanchor="middle",
        font=dict(family=FONT_UI, size=FS_NOTE, color=AC["text_muted"]))

    fig.update_yaxes(tickmode="array", tickvals=list(range(len(rows))),
                     ticktext=[m[1] for m in rows],
                     range=[len(rows) - 0.4, -0.95], showgrid=False,
                     zeroline=False, showline=False, ticklen=0,
                     tickfont=dict(family=FONT_UI, size=FS_AXIS,
                                   color=AC["text_primary"]))
    fig.update_xaxes(
        title=dict(text="forward transfer on one task, (AUC − "
                        "AUC<sub>base</sub>) / (1 − AUC<sub>base</sub>)",
                   font=dict(family=FONT_UI, size=FS_AXIS,
                             color=AC["text_primary"])),
        range=[min(-1.05, floor - 0.06), 1.32], dtick=0.5,
        showgrid=True, gridcolor=AC["grid"],
        gridwidth=0.6, zeroline=False, showline=True, linecolor=AC["axis"],
        linewidth=1.2, ticklen=4,
        tickfont=dict(family=FONT_MONO, size=FS_TICK, color=AC["text_muted"]))

    bottom = add_footnote(fig, (
        "One point per task; the heavy rule is the mean, which is the "
        "forward-transfer number reported everywhere else.<br>"
        "<b>The per-task spread is much wider than the gap between the "
        "means</b>, and the distributions overlap almost completely.<br>"
        "Ours and fine-tuning share a median of +0.12 despite different means, "
        "and every method makes 12 to 17 of the 50 tasks worse.<br>"
        "So the forward-transfer column orders the methods, but it does not "
        "separate them the way the retention columns do.<br>"
        f"One seed ({FWT_SEED}) for every method, so the point counts match."), 118)
    fig.update_layout(title=None, showlegend=False, plot_bgcolor=AC["bg"],
                      margin=dict(l=118, r=26, t=22, b=bottom))
    export_pair(fig, "forward_transfer_spread", W_FULL,
                46 * len(rows) + 22 + bottom)
...
def main() -> int:
    ...
    figure_forward_transfer()
    figure_forward_transfer_spread()
